...
def create_store(payload: schemas.StoreCreate, db: Session = Depends(get_db)):
    try:
        store = crud.create_store(db, payload)
...
@router.post("/{store_id}/devices", response_model=schemas.DeviceResponse, status_code=status.HTTP_201_CREATED)
def create_device(
    ...
    try:
        device = crud.create_device(db, store_id, payload)
    except LookupError as exc:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={"code": "store_not_found", "message": "La sucursal solicitada no existe."},
