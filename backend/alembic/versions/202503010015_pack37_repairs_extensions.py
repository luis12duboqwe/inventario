...
down_revision = "202503010014_cost_ledger"
branch_labels = None
depends_on = None
...
def upgrade() -> None:  # // [PACK37-backend]
    bind = op.get_bind()
    repair_part_source = sa.Enum("STOCK", "EXTERNAL", name="repair_part_source")
    repair_part_source.create(bind, checkfirst=True)

    with op.batch_alter_table("repair_orders", schema=None) as batch_op:
        batch_op.add_column(sa.Column("customer_contact", sa.String(length=120), nullable=True))
        ...
        batch_op.add_column(sa.Column("imei", sa.String(length=40), nullable=True))

    with op.batch_alter_table("repair_order_parts", schema=None) as batch_op:
        batch_op.add_column(sa.Column("part_name", sa.String(length=120), nullable=True))
        batch_op.add_column(
            sa.Column(
                "source",
