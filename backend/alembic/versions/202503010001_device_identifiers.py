...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("producto_id", sa.Integer(), sa.ForeignKey("devices.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.UniqueConstraint("producto_id", name="uq_device_identifiers_producto"),
        sa.UniqueConstraint("imei_1", name="uq_device_identifiers_imei_1"),
        sa.UniqueConstraint("imei_2", name="uq_device_identifiers_imei_2"),
        sa.UniqueConstraint("numero_serie", name="uq_device_identifiers_numero_serie"),
    )
    op.create_index(
...
def downgrade() -> None:
    op.drop_index("ix_device_identifiers_producto_id", table_name="device_identifiers")
