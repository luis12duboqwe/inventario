...
def upgrade() -> None:
    ...
    timestamp_default = sa.text("CURRENT_TIMESTAMP") if is_sqlite else sa.func.now()

    if not _table_exists(bind, "price_lists"):
        op.create_table(
            "price_lists",
    ...
    if not _table_exists(bind, "price_list_items"):
        op.create_table(
            "price_list_items",
            sa.Column("id", sa.Integer(), primary_key=True),
