"""Agregar tablas POS y totales de venta."""
...
def upgrade() -> None:
    ...
    op.execute("UPDATE sales SET subtotal_amount = total_amount, tax_amount = 0")
    ...
    op.create_table(
        ...
        sa.Column("invoice_prefix", sa.String(length=12), nullable=False, server_default="POS"),
        sa.Column("printer_name", sa.String(length=120), nullable=True),
        sa.Column("printer_profile", sa.String(length=255), nullable=True),
        sa.Column("quick_product_ids", sa.Text(), nullable=True),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.ForeignKeyConstraint(["store_id"], ["stores.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("store_id", name="uq_pos_config_store"),
    )
    op.create_index(op.f("ix_pos_config_id"), "pos_config", ["id"], unique=False)
    op.create_index(op.f("ix_pos_config_store_id"), "pos_config", ["store_id"], unique=False)
    ...
    op.alter_column("pos_config", "tax_rate", server_default=None)
    op.alter_column("pos_config", "invoice_prefix", server_default=None)


def downgrade() -> None:
    op.alter_column("pos_config", "invoice_prefix", server_default="POS")
    op.alter_column("pos_config", "tax_rate", server_default="0")
    ...
    op.drop_index(op.f("ix_pos_config_id"), table_name="pos_config")
