    ...
    >
      <div className="invoice-preview">
        <p className="muted-text">
          Descarga el comprobante en PDF para la venta #{saleId ?? "—"}. Verifica totales y motivo
          antes de confirmar.
        </p>
        {items.length === 0 ? (
          ...
          <ul className="compact-list">
            {items.map((line) => (
              <li key={line.device.id}>
                {line.device.sku} · {line.quantity} uds —{" "}
                {formatListCurrency(line.device.unit_price * line.quantity)}
              </li>
            ))}
          </ul>
        )}
