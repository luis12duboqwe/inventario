...
import { Skeleton } from "@components/ui/Skeleton"; // [PACK36-users-dashboard]
      ...
      <header className="user-dashboard__header">
        <div>
          ...
          <p className="card-subtitle">
            Actividad reciente, sesiones activas y alertas corporativas
          </p>
        </div>
        <button
          type="button"
              ...
              <ul className="user-dashboard__list">
                {recentActivity.map((item) => (
                  <li key={item.id ?? `${item.action}-${item.created_at}`}>
                    ...
                    />
                    <div>
                      <p className="user-dashboard__list-title">
                        {safeString(item.action, "Actividad corporativa")}
                      </p>
                      <p className="user-dashboard__list-meta">
                        {formatDateTime(item.created_at)} ·{" "}
                        {safeString(item.performed_by_name, "Sistema")}
                      </p>
                    </div>
                  </li>
                ))}
              ...
              <ul className="user-dashboard__list">
                {recentSessions.map((session) => (
                  <li key={session.session_id}>
                    <div
                      className={`status-indicator status-${session.status}`}
                      aria-hidden="true"
                    />
                    <div>
                      <p className="user-dashboard__list-title">
                        {safeString(session.username, "Usuario corporativo")}
                      </p>
                      <p className="user-dashboard__list-meta">
