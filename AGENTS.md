...
5. ✅ **Monitoreo Prometheus**: `/monitoring/metrics` (rol ADMIN) expone contadores de acuses, histogramas de tiempo de resolución, fallos y cache TTL (incluida la antigüedad del pendiente más antiguo) instrumentados en `backend/app/telemetry.py`; valida su salida cuando ejecutes pruebas de Seguridad.
