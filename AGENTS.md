...
- Backend: `process_smart_import` analiza encabezados libres, detecta IMEI, crea sucursales inexistentes y registra advertencias/patrones en `importaciones_temp`; además tolera `.csv` renombrados como `.xlsx` para evitar rechazos al leer archivos dañados. El historial (`/inventory/import/smart/history`) consolida columnas detectadas, registros incompletos y duración de cada corrida.【F:backend/app/services/inventory_smart_import.py†L16-L410】【F:backend/app/crud.py†L10135-L10168】
