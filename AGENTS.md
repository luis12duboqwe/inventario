...
- ✅ **Lote E — Seguridad y auditoría fina**: middleware global `X-Reason`, rutas `/security/*` (incluyendo `/2fa/status` protegido por flag), almacenamiento de secretos TOTP y sesiones activas (expuestas con IDs incrementales), componentes `TwoFactorSetup.tsx` y `AuditLog.tsx`.
...
1. Instrumentar telemetría de uso en cada sección del dashboard modular para priorizar mejoras de UX y redistribuir indicadores clave.
2. Elaborar el playbook de activación 2FA cuando `SOFTMOBILE_ENABLE_2FA` se habilite, contemplando el endpoint `/security/2fa/status` ya condicionado por flag.
3. Diseñar comparativas históricas adicionales en la sección de Analítica aprovechando los endpoints existentes como fuente base.
