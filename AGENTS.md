...
15. **Estructura de API**: La lógica de comunicación con el backend se encuentra modularizada en `frontend/src/api/` (con un `index.ts` como punto de entrada). Evita referencias a un archivo único `api.ts` que ya no existe.
...
- **Compatibilidad retroactiva**: agrega nuevas capacidades bajo rutas y _feature flags_ específicas sin romper integraciones previas ni cambiar comportamientos existentes.
...
- **Lotes funcionales a implementar**:
  ...
  - **Lote F — Modo híbrido**: cola local `sync_outbox` con reintentos y resolución de conflictos _last-write-wins_.
...
- **Frontend**: crear componentes React + TypeScript (`AdvancedSearch.tsx`, `TransferOrders.tsx`, `Purchases.tsx`, `Sales.tsx`, `Returns.tsx`, `AnalyticsBoard.tsx`, `TwoFactorSetup.tsx`, `AuditLog.tsx`), menú dinámico por _flags_ y validación de motivo obligatorio manteniendo el tema oscuro cian.
...
- ✅ **Lote F — Modo híbrido**: cola `sync_outbox` con reintentos, estrategia _last-write-wins_, panel de reintentos en `SyncPanel.tsx` y cobertura de pruebas.
...
- Propaga los datos extendidos al frontend: tipos actualizados en `frontend/src/api/index.ts`, helpers `getDeviceIdentifier`/`upsertDeviceIdentifier` y visualización en `InventoryTable.tsx` para IMEIs duales, serie extendida, estado técnico y observaciones.
...
- `frontend/src/api/index.ts` y `inventoryService.ts` exponen helpers (`getInventoryMovementsReport`, `downloadInventoryMovements{Csv|Pdf|Xlsx}`, etc.) que deben documentarse al añadir nuevos reportes.
...
- **28/10/2025 11:10 UTC** — El helper `listTransfers` en `frontend/src/api/index.ts` quedó fijado a `/transfers?limit=25&store_id=…` para eliminar redirecciones y asegurar que el panel de sincronización cargue datos consistentes desde la API.
...
- `frontend/src/api/index.ts` incorpora helpers `exportSalesPdf|Excel` y tipos enriquecidos (`SaleStoreSummary`, `SaleUserSummary`, `SaleDeviceSummary`); las pruebas `backend/tests/test_sales.py` verifican filtros y exportaciones.
...
- **28/10/2025 22:15 UTC** — Se ratificó que la pantalla presenta listado con filtros, formulario de creación/edición, toggles de estado activo/inactivo, checkboxes de asignación de roles y la matriz editable de permisos, además de botones de exportación PDF/Excel conectados a los endpoints de seguridad existentes.
...
- `frontend/src/api/index.ts` y `usersService` agregan funciones dedicadas a dashboard, permisos, exportaciones y CRUD, mientras que `frontend/src/styles.css` introduce grillas responsivas (`user-dashboard__*`, `stat-card`, `permissions-panel`, `user-form`) para mantener la estética Softmobile.
- Se añade el filtro «Bloqueados» (`status=locked`) a `/users`, `/users/export` y a la UI de `UserManagement.tsx`, mostrando indicadores visuales y totales actualizados para detectar cuentas con `locked_until` vigente sin afectar compatibilidad.
- El panel de usuarios ahora maneja métricas vacías sin fallos, ordena roles y permisos antes de renderizar para evitar mutaciones de estado y mantiene las columnas de la tabla en sincronía con la matriz de seguridad.
...
- `restore_backup` valida que los componentes solicitados existan en el respaldo antes de ejecutarse, admite restauraciones totales o parciales (base de datos, configuración, archivos críticos), copia archivos a destinos personalizados y aplica el SQL directamente cuando se requiera, registrando cada operación en `logs_sistema` mediante `crud.register_backup_restore` sin invalidar el job original.
...
- `GET /backups/{id}/download` permite exportar archivos `.zip`, `.sql` o `.json` de un respaldo, valida la existencia física, reutiliza el enum `BackupExportFormat` y mantiene la limitación al rol ADMIN para conservar la trazabilidad de datos sensibles.
...
- El helper `request` de `frontend/src/api/index.ts` agrega memoización con TTL de 60 segundos para peticiones GET, consolida solicitudes concurrentes bajo una única promesa y purga caché y pendientes tras operaciones mutables; `clearRequestCache` limpia ambos mecanismos para depuración y pruebas.【F:frontend/src/api/index.ts†L1586-L1750】
- La nueva suite `frontend/src/api.cache.test.ts` comprueba la reutilización de caché, la invalidación al ejecutar POST y la consolidación de peticiones simultáneas, certificando los controles de rendimiento exigidos para Softmobile 2025 v2.2.0.【F:frontend/src/api.cache.test.ts†L1-L142】
- `frontend/src/modules/operations/pages/OperationsPage.tsx` encapsula cada panel del acordeón en `React.lazy` y loaders accesibles, de modo que POS, transferencias e historial sólo se descargan al expandirlos y se elimina el costo de renders innecesarios en la carga inicial.【F:frontend/src/modules/operations/pages/OperationsPage.tsx†L1-L140】
- `frontend/src/modules/analytics/pages/AnalyticsPage.tsx` importa `AnalyticsBoard` de forma diferida y muestra un estado de carga consistente con el tema corporativo mientras se descarga el paquete de gráficas pesadas.【F:frontend/src/modules/analytics/pages/AnalyticsPage.tsx†L1-L80】
- `frontend/src/modules/reports/pages/GlobalReportsPage.tsx` delega `GlobalReportsDashboard` a un `React.lazy` con loader accesible, descargando alertas y exportaciones únicamente cuando se visita la sección de reportes globales.【F:frontend/src/modules/reports/pages/GlobalReportsPage.tsx†L1-L44】
- Se documentan pruebas de rendimiento dedicadas: `OperationsPage.lazy.test.tsx` valida la carga bajo demanda de cada sección y `App.performance.test.tsx` garantiza que el arranque de la UI permanece debajo de los 2 segundos.【F:frontend/src/modules/operations/pages/OperationsPage.lazy.test.tsx†L1-L88】【F:frontend/src/App.performance.test.tsx†L1-L18】
- `frontend/src/modules/inventory/pages/InventoryPage.tsx` convierte AdvancedSearch, InventoryTable, MovementForm, InventoryReportsPanel y el diálogo de edición en imports perezosos, agrega loaders accesibles y memoiza tarjetas/acciones con `useMemo`/`useCallback` para evitar renders redundantes al refrescar métricas o filtros.【F:frontend/src/modules/inventory/pages/InventoryPage.tsx†L1-L1208】
...
- Se añaden pruebas Vitest (`AdminControlPanel.test.tsx`, `ActionIndicatorBar.test.tsx`) para validar accesos, mensajes y estados visibles del nuevo panel administrativo.【F:frontend/src/modules/dashboard/components/**tests**/AdminControlPanel.test.tsx†L1-L40】【F:frontend/src/modules/dashboard/components/**tests**/ActionIndicatorBar.test.tsx†L1-L40】
...
- **02/11/2025 23:10 UTC** — El panel añade `NotificationCenter` con resumen accesible, apertura automática ante incidencias, badges con variantes `warning/danger/info` y un enlace «Saltar al contenido principal» para navegación por teclado. Documentado en README/CHANGELOG/AGENTS bajo «Actualización Interfaz - Parte 3 (Panel, Usabilidad y Accesibilidad)».【F:frontend/src/modules/dashboard/components/NotificationCenter.tsx†L1-L79】【F:frontend/src/modules/dashboard/components/AdminControlPanel.tsx†L1-L103】【F:frontend/src/modules/dashboard/layout/DashboardLayout.tsx†L33-L270】【F:frontend/src/styles.css†L180-L213】【F:frontend/src/styles.css†L4829-L4916】【F:README.md†L107-L116】【F:CHANGELOG.md†L58-L66】
- **02/11/2025 23:55 UTC** — Se refuerza la orientación del panel marcando el módulo activo con `aria-current`, sumando mensajes para lectores de pantalla, badges contextuales y reglas `prefers-reduced-motion` que atenúan animaciones en el centro de control. La documentación se actualiza en README/CHANGELOG/AGENTS bajo «Actualización Interfaz - Parte 3 (Panel, Usabilidad y Accesibilidad)».【F:frontend/src/modules/dashboard/components/AdminControlPanel.tsx†L1-L129】【F:frontend/src/modules/dashboard/layout/DashboardLayout.tsx†L33-L280】【F:frontend/src/styles.css†L4746-L5017】【F:README.md†L107-L117】【F:CHANGELOG.md†L58-L69】
...
### Importación Inteligente desde Excel – v2.2.0 implementada y verificada (22/02/2025 18:30 UTC)

- Backend: `process_smart_import` analiza encabezados libres, detecta IMEI, interpreta columnas booleanas expresadas como «sí/no», `true/false` o `1/0`, crea sucursales inexistentes y registra advertencias/patrones en `importaciones_temp`; además tolera `.csv` renombrados como `.xlsx` para evitar rechazos al leer archivos dañados. El historial (`/inventory/import/smart/history`) consolida columnas detectadas, registros incompletos y duración de cada corrida.【F:backend/app/services/inventory_smart_import.py†L16-L453】【F:backend/app/crud.py†L10135-L10168】
