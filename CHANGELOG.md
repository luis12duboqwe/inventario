...
- `_log_action` ahora deriva el módulo, usuario y nivel de severidad (`info`, `warning`, `critical`) y replica la entrada en `logs_sistema`, mientras que `register_system_error` almacena incidentes en `errores_sistema` y añade registros `system_error` con nivel `error`.【F:backend/app/crud.py†L332-L421】【F:backend/app/crud.py†L339-L418】
...
- La prueba `backend/tests/test_system_logs.py` verifica que las acciones clave de ventas, inventario y auditoría generen entradas con severidades `info`, `warning`, `critical` y que los errores se registren correctamente con nivel `error`, incluyendo filtros por usuario, módulo y fechas futuras vacías.【F:backend/tests/test_system_logs.py†L1-L96】
