...
- **Reportes dedicados en backend**: nuevos endpoints `GET /reports/inventory/current`, `/value`, `/movements` y `/top-products` entregan existencias consolidadas, valoración por tienda, movimientos filtrables por periodo y ranking de productos vendidos. Cada ruta cuenta con exportación CSV (`/csv`) que exige `X-Reason` y roles de reporte.
- **Exportación de existencias consolidadas**: `GET /reports/inventory/current/csv` genera un resumen por sucursal con dispositivos, unidades y valor total, reutilizando los agregadores del backend y aplicando filtros opcionales por tienda. El frontend añade el botón "Exportar CSV" en la tarjeta de existencias y centraliza la descarga vía `downloadInventoryCurrentCsv`, asegurado por la prueba `InventoryPage.test.tsx`.
...
- **Nuevo tab de reportes en frontend**: `InventoryPage.tsx` integra el componente `InventoryReportsPanel.tsx`, mostrando existencias, valoración y movimientos en tarjetas temáticas con filtros por sucursal y rango de fechas, además de botones de exportación.
- **SDK y servicios actualizados**: `frontend/src/api.ts` ofrece funciones `getInventoryCurrentReport`, `getInventoryMovementsReport`, `downloadInventoryMovementsCsv`, entre otras, utilizadas por `inventoryService.ts` para centralizar descargas y consultas.
...
- **Cobertura de UI**: la suite `InventoryPage.test.tsx` asegura la renderización del nuevo tab y que las exportaciones invoquen la captura de motivo corporativo antes de disparar las descargas.
