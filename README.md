...
- ✅ **Monitoreo de auditoría vía Prometheus**: el backend expone `/monitoring/metrics` (rol ADMIN) con contadores de acuses, histogramas de tiempo de resolución, fallos e indicadores de cache TTL (incluida la antigüedad del pendiente más antiguo) instrumentados con `prometheus-client`, centralizados en `telemetry.py` y cubiertos por `test_prometheus_metrics_endpoint`.【F:backend/app/routers/monitoring.py†L1-L32】【F:backend/app/telemetry.py†L1-L118】【F:backend/tests/test_audit_logs.py†L1-L220】
