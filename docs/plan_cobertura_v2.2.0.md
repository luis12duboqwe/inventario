...
- **Observabilidad**
  5. ✅ `/monitoring/metrics` queda disponible para roles `ADMIN`, exponiendo contadores de acuses, histogramas de tiempo de resolución, fallos e hit/miss de cache (con gauge de antigüedad del pendiente más antiguo) definidos en `backend/app/telemetry.py`; consulta su salida después de cada corrida de pruebas para garantizar coherencia con los datos mostrados en Seguridad.
